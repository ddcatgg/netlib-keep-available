...
-  `WECOM_WEBHOOK_URL`    企业微信通知推送链接
4. GitHub Actions 初始手动执行一次检查是否有配置错误，脚本会自动定期执行，也可手动执行。

### 注意事项

1. 确保 Netlib 账户密码正确
2. 首次运行 GitHub Actions 需要授权
3. 脚本执行时间为 UTC 12:00（香港时间 20:00）
4. 如果不需要 Telegram、企微通知，可不配置相关环境变量
